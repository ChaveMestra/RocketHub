...
public class Holos {
    ...
    private void updateHolos() throws SQLException {
        ...
        count = 0;
        ResultSet topParkourRs = dbManager.executeQueryLivre("SELECT name,tempoParkour FROM Rocket_Hub ORDER BY tempoParkour ASC");
        System.out.println("Começando upd de parkour");
        while (topParkourRs.next()) {
            if (topParkourRs.getInt("tempoParkour") != 0 && count < 5) {
                System.out.println("Passei pelo player " + topParkourRs.getString("name"));
                count++;
