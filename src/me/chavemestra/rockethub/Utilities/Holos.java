...
public class Holos {
    ...
    public Holos(Plugin p) {
        try {
            ...
            topKills = HologramsAPI.createHologram(p, new Location(Bukkit.getWorld("Lobby"), 5, 5, 5));
            topParkour = HologramsAPI.createHologram(p, new Location(Bukkit.getWorld("Lobby"), 10, 5, 10));
    ...
    private void updateHolos() throws SQLException {
        ...
        ResultSet topParkourRs = dbManager.executeQueryLivre("SELECT name,tempoParkour FROM Rocket_Hub ORDER BY kills ASC limit 5");
        int count = 0;
        while (topKillsRs.next()) {
            count++;
            topKills.appendTextLine(f("&5#&d" + count + " &fJogador: &e" + topKillsRs.getString("name") + " &fKills: " + topKillsRs.getInt("kills")));
        }
        count = 0;
        while (topParkourRs.next()) {
            count++;
            topParkour.appendTextLine(f("&5#&d" + count + " &fJogador: &e" + topParkourRs.getString("name") + " &fKills: " + topParkourRs.getInt("tempoParkour") + "s"));
        }
    }
